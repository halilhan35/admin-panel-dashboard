...
router.post("/auth", async(req, res) => {
  try {
    ...
    // Eğer Users.js içinde validateFieldsBeforeAuth metodunun başına "static" eklediysen sorunsuz çalışır 🚀
    Users.validateFieldsBeforeAuth(email, password);

    let user = await Users.findOne({email});
    if(!user) throw new CustomError(Enum.HTTP_CODES.UNAUTHORIZED, "Authentication Error", "email or password wrong");

    if(!user.validPassword(password)) throw new CustomError(Enum.HTTP_CODES.UNAUTHORIZED, "Authentication Error", "email or password wrong");
    ...
    const statusCode = (errorResponse && errorResponse.code) ? errorResponse.code : 500;
    const responseData = errorResponse || { error: "Internal Server Error", message: err.message };
